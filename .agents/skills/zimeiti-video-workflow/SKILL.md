...
- Formal public packages need `publish-copy-v1`: a final-script viewer brief, an 8+ candidate / 4+ angle title matrix, exact selected-title binding, body/comment continuation, and a fresh independent scorecard validated by `scripts\test-video-publish-copy.ps1`.
...
Publishing-copy contract check:

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\test-video-publish-copy.ps1 -VideoDir <video-dir>
```
